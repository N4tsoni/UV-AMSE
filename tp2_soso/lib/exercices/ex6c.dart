...
class Tile {
  Color color = Color.fromARGB(255, 11, 37, 205);
  String text;
  int position;
      ...
      child: Container(
        decoration: BoxDecoration(
          color: widget.currentWhiteTile == widget.tile.position
              ? Colors.white
              : widget.tile.color,
          border: isNeighbor ? Border.all(color: Color.fromARGB(255, 0, 0, 0), width: 5) : null,
        ),
        child: Center(
          child: Text(
            widget.tile.text,
...
class _Ex6cState extends State<Ex6c> {
    ...
    setState(() {
     bool isNeighbor = neighbors[currentWhiteTile]?.contains(tileNumber) ?? false;
      if (currentWhiteTile == tileNumber) {
        ...
        int a = tileNumber;
        swap(texts, currentWhiteTile, tileNumber);
        currentWhiteTile = a;
      } else {
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      appBar: _buildAppBar(),
      backgroundColor: Color.fromARGB(255, 225, 34, 56),
      body: Center(
        child: Container(
          height: 500,
          width: 500,
...
AppBar _buildAppBar() {
  return AppBar(
    title: Text(
      'Exercice 6C: bouger des tuiles dans plateau',
      style: TextStyle(
