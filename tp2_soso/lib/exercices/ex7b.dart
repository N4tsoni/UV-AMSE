...
import 'package:tp2_soso/classes/AStar.dart';
import 'package:tp2_soso/classes/Taquin.dart';
...
const double initialSliderValue = 3.0;
    ...
    if (Imageasset) {
      ...
      return FittedBox(
        ...
        child: ClipRect(
          child: Container(
            decoration: BoxDecoration(
                border: Border.all(color: const Color.fromARGB(255, 82, 54, 244)), // Add a red border
              ),
              child: Align(
                alignment: this.alignment,
                widthFactor: this.widthFactor,
...
class Ex7b extends StatefulWidget {
  final Exercice exercice;

  const Ex7b({Key? key, required this.exercice}) : super(key: key);

  @override
  _Ex7bState createState() => _Ex7bState();
}

class _Ex7bState extends State<Ex7b> {
  bool isPlaying = false;
  int currentWhiteTile = -1;

// Extracted method to reset the game state
...
late List<Widget> tiles;
  ...
  void initState() {
    ...
    texts = definition_texts(initialSliderValue.toInt());
...
// Extracted method to generate tiles based on the provided texts
List<Widget> generateTiles(List<String> liste_texts) {
  ...
  for (int i = 1; i <= _currentSliderValue.toInt() * _currentSliderValue.toInt(); i++) {
    ...
    if (Imageasset || ImageInternet) {
      if (liste_texts[i - 1] == "") {
        tile = Tile(Colors.white, "", i); // White tile
      } else {
      ...
      tile = Tile(Color.fromARGB(125, random.nextInt(255), random.nextInt(255), random.nextInt(255)), liste_texts[i - 1], i);
    }
...
// Extracted method to handle tile taps
void _handleTileTap(int tileNumber) {
  ...
  // Extracted method to get a message based on the difficulty value
...
AppBar _buildAppBar() {
  ...
  // Extracted method to build game controls
